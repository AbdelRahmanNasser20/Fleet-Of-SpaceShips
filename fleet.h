...
private:
    ...
    Ship* ins(Ship* root, const Ship &ship);
    void listShipsInOrder(Ship* root) const;
    Ship* findShipRec(Ship* root, int id) const;
    void clearRec(Ship* root);
    int max(int a, int b );
    int height(Ship * root);

    Ship* deleteRecu(Ship* ship, int id);
    Ship* removeRecu(Ship* root, int id);
    void findLostShip(Ship* root);

    Ship* rightRotation(Ship* ship);
    Ship * leftRotation(Ship* ship);

    Ship* leftRecu(Ship* ship);
